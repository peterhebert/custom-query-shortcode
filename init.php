...
class Query_Shortcode {
	 ...
	 */
	public function shortcode( array|string $atts = '' ) {
		if ( ! is_array( $atts ) ) {
			return;
		}
		...
		foreach ( $theme_args as $key => $value ) {
			unset( $query[ $key ] );
		}
		...
		*/
		if ( array_key_exists( 'twig_template', $atts ) && class_exists( 'Timber' ) ) {
			...
			if ( $template_exists ) {
				...
				$output = Timber::compile( $safe_twig_filename, array( 'posts' => $timber_post_data ) );
			} else {
				...
				if ( $posts->have_posts() ) :
					while ( $posts->have_posts() ) :
								...
								$keywords     = apply_filters(
									'query_shortcode_keywords',
									$keyword_vals
								);
							$the_items[]      = $this->get_block_template( $template, $keywords );
					endwhile;
				endif;

				if ( array_key_exists( 'shortcode', $atts ) && true === boolval( $atts['shortcode'] ) ) {
					array_map( 'do_shortcode', $output );
				}

				if ( ( array_key_exists( 'cols', $atts ) && $atts['cols'] > 1 ) || ( array_key_exists( 'rows', $atts ) && $atts['rows'] > 1 ) ) {
					$grid = $this->build_grid( $the_items, $atts['cols'], $atts['rows'] );
					echo wp_kses( $grid, wp_kses_allowed_html() );
				} else {
					$separator = array_key_exists( 'posts_separator', $atts ) ? $atts['posts_separator'] : ' ';
					echo wp_kses( implode( $separator, $the_items ), wp_kses_allowed_html() );
				}
			}
	 ...
	 */
	private function get_block_template( string $text, array $parameters = array() ) {
		$searches     = array();
		$replacements = array();
